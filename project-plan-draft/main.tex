...
Classification problems involve using an algorithm, and a given set of inputs used as trained features, to classify any given input with a predefined label. In other words, Classification problems involve searching for a discrete target value, often among a subset of discrete values, whereas Regression problems involve finding a continuous target value. \par
In this project, the aims will be to focus specifically on Classification problems solved by Supervised Learning, evaluate how they function, and compare how several algorithms perform amongst each other. \par
The first algorithm I'll be exploring will be the Nearest Neighbours~\cite{NNCoverHart} algorithm, one of the simplest learning algorithms. I'll be attempting to compare this implementation with an implementation of the Decision Tree algorithm implementation. \par
...
I intend on using openly available datasets found on the UCI Repository~\cite{uci}, including the Delve datasets, as well as datasets from open-source communities such as Kaggle, OpenML and huggingface for further training. Datasets from the latter are likely to require more preprocessing and normalisation. \par
